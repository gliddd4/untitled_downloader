...
def should_keep_original(file_path, source_ext="", content_type=""):
...
def convert_or_keep_audio(temp_path, output_base_path, source_ext="", content_type=""):
    keep_original, reason = should_keep_original(temp_path, source_ext=source_ext, content_type=content_type)
    ...
    output_path = f"{output_base_path}.mp3"
    ffmpeg_cmd = [
    ...
    os.remove(temp_path)
    return output_path, "Converted to MP3"
...
def scrape_untitled_track(track, dir_path, track_title):
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as r:
            ...
            mp3_path = os.path.join(dir_path, f"{sanitized_track}.mp3")
            if os.path.exists(mp3_path):
                return f"Skipped existing: {mp3_path}"
